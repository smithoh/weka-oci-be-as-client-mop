...
[![Step 2](https://github.com/smithoh/weka-oci-be-as-client-mop/raw/main/images/step2.png)](/smithoh/weka-oci-be-as-client-mop/blob/main/images/step2.png)
...
[![Step 4](https://github.com/smithoh/weka-oci-be-as-client-mop/raw/main/images/step4.png)](/smithoh/weka-oci-be-as-client-mop/blob/main/images/step4.png)
...
[![Step 5](https://github.com/smithoh/weka-oci-be-as-client-mop/raw/main/images/step5.png)](/smithoh/weka-oci-be-as-client-mop/blob/main/images/step5.png)
...
date
```
> 옵션은 저장소의 [`fio.job`](https://github.com/smithoh/weka-oci-be-as-client-mop/blob/main/fio.job) 기준. `stonewall`은 단일 실행이라 생략, `filename_format=$jobnum`은 셸 확장 방지로 작은따옴표.
> 기대: 각 실행 끝에 `READ:`/`WRITE:` bw·IOPS 요약 출력 (seq=대역폭, rnd=IOPS 관점).

## Step 7. Stats 수집 (fio 구간) [T]

Step 6에서 실행한 fio 4종 테스트 구간의 WEKA 통계를 수집합니다. START/END는 여기서 한 번만 고정해서 이후 모든 명령이 동일한 구간을 가리키도록 합니다 (fio 4종 총 실행 시간이 약 12분이라 15분 창이면 앞뒤 buffer까지 포함됩니다).

```
END=$(date +%Y-%m-%dT%H:%M:%S)
START=$(date -d "-15 minutes" +%Y-%m-%dT%H:%M:%S)
```
> 노드 타임존이 KST이므로 Z(UTC 표기)는 붙이지 않습니다. Z를 붙이면 UTC로 해석되어 실제 로컬 시각보다 9시간 뒤 구간을 수집하게 됩니다.
...
**RPC Latency** (NodeId < 10,000 = backend / NodeId >= 10,000 = client — 분석 시 구분)
...
```
> 기대: 위 파일들(약 50개)이 현재 디렉토리에 생성됨. 전부 동일한 15분 구간을 가리키므로, latency/drive/network 파일을 서로 겹쳐보며 병목 구간을 특정할 수 있음.
> 위 전체 명령은 저장소의 [`stats-collect.sh`](https://github.com/smithoh/weka-oci-be-as-client-mop/blob/main/stats-collect.sh)로도 제공됩니다 (`sh stats-collect.sh` 한 번 실행으로 동일 수집 가능).
...
[![Step 7](https://github.com/smithoh/weka-oci-be-as-client-mop/raw/main/images/step7.png)](/smithoh/weka-oci-be-as-client-mop/blob/main/images/step7.png)

## Step 9. Re-add — target를 backend로 복원 [T]

### 9-1. 데이터 드라이브 확인 후 wipe (stale 시그니처 제거)
...
[![Step 8-2](https://github.com/smithoh/weka-oci-be-as-client-mop/raw/main/images/step8-2.png)](/smithoh/weka-oci-be-as-client-mop/blob/main/images/step8-2.png)

### 9-3. 새 drives0 컨테이너 ID 확인 후 드라이브 재추가 [C]
